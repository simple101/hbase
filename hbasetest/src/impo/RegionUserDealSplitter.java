 ...
 * @date 2013-7-1
 */
public class RegionUserDealSplitter {
	static final Log logger = LogFactory.getLog(RegionUserDealSplitter.class);
	//获取配置文件参数
	...
	//获取列族
	static String famliy = pro.getProperty("table.wap.family");
	//分发处理器类
	 static String[] coprocessor_class = pro.getProperty("table.wap.coprocessor.class").split(",");
	 //分发处理器地址
	 static String[] coprocessor_path = pro.getProperty("table.wap.coprocessor.path").split(",");
	 //压缩方式
	 static String compression = pro.getProperty("table.wap.compression");
	//表描述方式
	 static String tableDescriptor = pro.getProperty("table.wap.descriptor");
	 
	 //Region数量
	 static int sprangsize = Integer.parseInt(pro.getProperty("table.wap.region.size"));
	 //key的长度
	 static int keylength = Integer.parseInt(pro.getProperty("table.wap.key.length"));
